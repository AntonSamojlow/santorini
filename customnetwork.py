...
class DenseModel():
    ...
    def __init__(self,
        ...
        for layer_size in layer_sizes[1:-1]:
            if layer_size == "BN":
                ...
                layers.append(
                    tf.keras.layers.Dropout(dropout_rate)(layers[-1]))

        out_pi = tf.keras.layers.Dense(layer_sizes[-1][0],
                                       activation='softmax',
                                       name='pi')(layers[-1])
        out_v = tf.keras.layers.Dense(1, activation='tanh',
                                      name='v')(layers[-1])
        self.model = tf.keras.models.Model(inputs=layers[0],
    ...
    def fit(self, train_config):
        ...
        LOGGER.info(
            "loss on full data (train+validation) before training: {0:.5f} (pi={1:.5f}|v={2:.5f})"
            .format(vals[0], vals[1], vals[2]))

        trainresult = self.model.fit(
            train_config.ds_train,
        ...
        print()
        LOGGER.info(
            "loss on full data (train+validation) after training: {0:.5f} (pi={1:.5f}|v={2:.5f})"
            .format(vals[0], vals[1], vals[2]))
        t1 = time()
...
class TrainConfig():
    ...
    def LoadDatasets(self, folderpath=None, x=None, y_pi=None, y_val=None):
        ...
        if folderpath is not None:
            ...
            y_val = np.loadtxt(path.join(folderpath, 'y_val.csv'),
                               delimiter=',')
        ...
        LOGGER.info(
            'TrainConfig loaded and prepares {0} samples of which {1} are resevered for validation'
        ...
        self.ds_validate = dataset.take(n_validation).cache().batch(
            self.batchsize)
        self.ds_train = dataset.skip(n_validation).take(
            n_train).cache().repeat().batch(self.batchsize)
...
class EpochDots(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs):
        if epoch % self.report_every == 0:
            ...
            print(', val loss: {:0.5f} (pi {:0.5f}, v {:0.5f})'.format(
                logs['val_loss'], logs['val_pi_loss'], logs['val_v_loss']),
                  end='')
            print()
...
class Tools():
    ...
    class HistoryPlotter(object):
        ...
        def plot(self, histories, metric, figsize=(8, 6), savepath=None):
            ...
            plt.xlim([
                0,
                max([history.epoch[-1] for history in histories.values()])
            ])
            y_min = min([
            ...
            if savepath is not None:
                if path.exists(savepath):
                    LOGGER.warning(
                        "overriding existing plot '{}'".format(savepath))
