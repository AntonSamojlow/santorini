...
class Node():
    ...
    Assumptions/Notes:
        ...
        4.  A different initial value can be set by overriding __init__,
            BUT the value should be in [-1, +1] and conform with 2 above.
    ...
    def print(self, max_depth=None, __current_depth=0, value_digits=3):
        ...
        node_info = str(str(self.name) + ' ('
                        + str(round(self.value, value_digits)
                              ).ljust(3+value_digits, ' ')
                        + '|' + str(self.visits) + ')')
...
class Alphabeta():
    ...
    def tabled(self, node, depth, _alpha=-1, _beta=1,
        ...
        if node.name in self.table.keys():
            (depth_table, flag, score, play) = self.table[node.name]
        ...
        if bestval <= _alpha:    # 'beta'-cutoff appeared for some child
            ...
            flag = 'lowerbound'
        self.table[node.name] = (depth_table, flag, score, play)
