        ...
        try:
            ...
            def try_load_datasets() -> tuple:
                ...
                activefolders = [f for f in os.scandir(self.gympath.gamerecordpool_folder) 
                    if f.is_dir() and int(f.name) + self.config.max_sampleage >= modeliteration]
                basepaths = []
                ...
                self.logger.debug("loaded {} samples from files - n_train={}, n_validate={}".format(int(x.shape[0]), n_train, n_validate))
                if n_train < self.config.batchsize: # should one also check n_validate?
                    return Exception("Less training samples ({}) than training batchsize ({})".format(n_train, self.config.batchsize))
                ...
                if n_validate > 0:
                    ds_validate = dataset.take(n_validate).cache().repeat().batch(self.config.batchsize)
                else:
                    ds_validate = None
                ds_train = dataset.skip(n_validate).take(n_train).cache().repeat().batch(self.config.batchsize)
            ...
            with tf.device(tf_device):            
                ...
                self.logger.info("tf.keras.model (iteration {}) loaded from {}".format(model_iteration, self.gympath.model_folder))
                while not self.endevent.is_set():
                    if self.newmodelevent.is_set():
                        ...
                        if isinstance(load_datasets_result, Exception):
                            self.logger.debug("failed loading dataset: {}".format(load_datasets_result))
                            ...
                            if n_train < self.config.min_samplecount:
                                self.logger.debug("training dataset ({} entries) below min_samplecount ({}) - sleeping 30 sec".format(
                                    n_train, self.config.min_samplecount))
                                sleep(30)
                            else:
                                self.logger.debug("loaded dataset: {}".format(ds_train.element_spec))                            
                                trainresult = MODEL.fit(ds_train,
                                                steps_per_epoch=1+n_train//self.config.batchsize,
                                                validation_data=ds_validate,
                                                validation_steps= 1+n_validate//self.config.batchsize,
                                ...
                                model_iteration+=1                        
                                with open(self.gympath.modelinfo_file, 'w+') as f:
                                ...
                                history_dump_file = os.path.join(self.gympath.trainhistories_folder,"{}.json".format(model_iteration))
...
class HistoryPlotter(object):
    ...
    def __init__(self, logger:logging.Logger, smoothing_std=None,):
        self.color_table = {}
    ...
    def plot(self, histories, metric, figsize=(20, 10), savepath=None):
        ...
        if savepath is not None:   
            if os.path.exists(savepath):
                self.logger.warning("HistoryPlotter overriding existing plot '{}'".format(savepath))
            directory = os.path.dirname(savepath)
            if not os.path.exists(directory):
                self.logger.debug("HistoryPlotter creating directory '{}'".format(directory))
                os.makedirs(directory)
            plt.savefig(savepath)
            self.logger.debug("HistoryPlotter saved plot to '{}'".format(savepath))
